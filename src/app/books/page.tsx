...
export default async function BooksCatalogPage({ searchParams }: PageProps) {
  ...
  // Fetch page data (only category-based filtering is allowed now)
  const [{ books, totalPages }, categories, user] = await Promise.all([
    getBooks({ page, limit: 12, search: "", categoryId }),
    getCategories(),
    getCurrentUser(),
  ])
